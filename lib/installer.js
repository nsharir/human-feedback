...
const HERMES_MEMORY_RULE = [
  ...
  'agent-feedback (always-on, even if plugin hook silent): if you would ask ≥2 substantive questions in one reply, STOP and write `questions*.json` (filename must start with "questions") instead. Review artifacts — including UX mockups, design comps, prototypes, previews, specs, plans, proposals — MUST be named `<topic>-review.md` / `<topic>-review.html` AND live under `<workspace>/.hermes/plans/<filename>` (where `<workspace>` is the absolute path from the most recent `[Workspace::v1: /abs/path]` tag on the user\'s message — NOT `/tmp`, NOT the repo root, NOT `$HOME`, NOT `mocks/`, NOT `mockups/`, NOT `previews/`, NOT `docs/`, NOT any project subdirectory). The auto-wrap hook ONLY fires for that exact suffix in that exact directory; anywhere else stays silent and your `MEDIA:` token will point at a non-existent compiled file. If the user wants the artifact also stored inside the project, copy it AFTER the review handoff, not instead. Reply ONLY: `questionnaire ready ✓ — open: <file://link> — waiting on your response.` (or `review ready ✓ — …`). On Hermes WebUI also append a `MEDIA:<wrapped-output-path>` line in the reply — WebUI renders the wrapped HTML inline as a sandboxed iframe so the user does not need to navigate. Never `open` from terminal — relay the link the hook printed. Manual fallback when the hook is silent: `agent-feedback compile <input> -o <output> --force`, then share the path as a file:// link and a MEDIA: token.',
...
// Append the rule entry to MEMORY.md, idempotently. If a managed block
// already exists with stale content (e.g. an older release shipped a
// shorter rule), replace it in place — this keeps the rule current
// across `agent-feedback` upgrades without requiring the user to
// uninstall + reinstall.
...
function installHermesMemoryRule(scope) {
  ...
  // First-time install: no managed block yet — append as a fresh entry.
  if (!existing.includes(HERMES_MEMORY_MARKER_BEGIN)) {
  ...
  // Upgrade path: managed block present — replace its body with the
  // current rule text if it has drifted. We rewrite ONLY the bytes
  // between the markers (keeping any leading "§\n" separator the
  // original install inserted) so neighbouring user entries are
  // untouched.
  ...
  if (currentBlock === HERMES_MEMORY_RULE) return false;

  const next = existing.replace(blockRe, HERMES_MEMORY_RULE);
  fs.writeFileSync(file, next);
  return true;
}
...
// Read the version field from a plugin.json that may or may not exist.
// Returns null when the file is missing or malformed — callers treat that
// as "unknown / older than anything bundled" and trigger an overwrite.
function readPluginVersion(target) {
  try {
...
// Compare two semver-ish strings ("1.7.4" vs "1.1.0"). Returns
//  -1 if a < b, 0 if equal, 1 if a > b. Non-numeric segments fall back
// to a lexicographic compare so prereleases ("1.7.4-beta.1") still order
// sensibly enough for our "is the installed copy stale?" gate.
function compareVersions(a, b) {
  ...
  for (let i = 0; i < len; i++) {
    const na = parseInt(segA[i] || '0', 10);
    const nb = parseInt(segB[i] || '0', 10);
...
function installHermes(scope) {
  ...
  const bundledVersion = readPluginVersion(HERMES_SRC);
  const installedVersion = pluginAlready ? readPluginVersion(target) : null;

  // Three branches:
  //   1. Fresh install      → copy everything in.
  //   2. Newer bundled      → overwrite in place (auto-upgrade). This
  //                           catches the failure mode where the CLI is
  //                           updated globally but the in-place plugin
  //                           directory still ships the version it had
  //                           the day the user first ran `install`.
  //   3. Same / older       → skip; treat the existing directory as
  //                           authoritative. Memory rule still runs.
  let pluginAction = 'skipped';
  if (!pluginAlready) {
    copyPluginInto(target);
    pluginAction = 'installed';
  } else if (compareVersions(bundledVersion, installedVersion) > 0) {
    copyPluginInto(target);
    pluginAction = 'upgraded';
  }
  ...
  // this, the rule never reaches the agent in that state. (v1.7.4: also
  // rewrites the block in place when the rule text has drifted.)
  const memoryWrote = installHermesMemoryRule(scope);

  return {
    changed: pluginAction !== 'skipped' || memoryWrote,
    target,
    plugin: {
      action: pluginAction,
